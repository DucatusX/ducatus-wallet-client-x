...
API.prototype.validateKeyDerivation = function(opts, cb) {
  ...
  // THESE ARE LITECOIN KEYS, REPLACE WITH DUCATUS KEYS
...
API.prototype.getFeeLevels = function(coin, network, cb) {
  ...
  //$.checkArgument(coin || _.includes(['btc', 'bch'], coin));                                                    //$.checkArgument(network || _.includes(['livenet', 'testnet'], network));
  // TODO: don't hardcode this, the above is causing an over-under-flow
  coin = 'btc';
  network = 'livenet';

  log.error('/v2/feelevels/?coin=' + (coin || 'btc') + '&network=' + (network || 'livenet'));
